...
const RoomPaymentForm = ({ clientSecret, handleSetPaymentSuccess }: RoomPaymentFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const { data: bookings } = await axios.get(`/api/booking/${bookingRoomData.room.id}`);
            ...
            if (!error && paymentIntent) {
                await axios.patch(`/api/booking/${paymentIntent.id}`);
    ...
    return (
        <form onSubmit={handleSubmit} id="payment-form">
            ...
            {isLoading && (
                <Alert className='bg-indigo-600 text-white mb-4'>
